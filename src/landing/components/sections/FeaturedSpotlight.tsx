      ...
      <div className="max-w-[1200px] mx-auto px-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-[0.9fr_1.1fr] border border-border-strong bg-bg-raised">
          ...
          <div className="p-10 flex flex-col">
            <h3 className="text-3xl font-bold font-plus mt-3.5 mb-3.5">
              {featuredRaffle.title}
            </h3>
            <p className="text-base text-text-secondary font-plus leading-relaxed mb-6">
              {featuredRaffle.description}
            </p>

            <div className="grid grid-cols-3 gap-px bg-border border border-border mb-7">
              {specs.map((spec) => (
                <div key={spec.label} className="bg-bg-raised px-4 py-3.5">
                  <div className="text-[10px] font-plus text-text-muted uppercase tracking-wide mb-1">
                    {spec.label}
                  </div>
                  <div className="font-plus text-base font-bold text-brass-light">
                    {spec.value}
                  </div>
                </div>
              ))}
