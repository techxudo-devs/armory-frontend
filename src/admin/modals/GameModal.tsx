...
export function GameModal({ isOpen, type, game, onClose, onSubmit, isLoading = false }: GameModalProps) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 p-4 backdrop-blur-sm">
      <div className="flex max-h-[80vh] w-full max-w-lg flex-col rounded-2xl border border-[#3D2715] bg-[#24140B] p-5 text-[#F4EADD] shadow-2xl shadow-black/60">
        ...
        <form onSubmit={handleSubmit} className="space-y-4 overflow-y-auto pr-1">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div>
              ...
              {!isViewOnly && (
                <label
                  htmlFor="prizeImage"
                  className="flex cursor-pointer items-center gap-3 rounded-xl border border-dashed border-[#3D2715] bg-[#24140B] px-3.5 py-2.5 text-sm text-[#C09A76] transition-colors duration-300 hover:border-[#D29A45] hover:text-[#E3C49A]"
                >
              ...
              {values.prizeImageUrl && (type === 'edit' || type === 'view') && (
                <Image
                  ...
                  width={64}
                  height={64}
                  className="mt-2 h-16 w-16 rounded-lg border border-[#3D2715] object-cover"
                />
              )}
            </div>
          </div>
          ...
          {isViewOnly && (
            <button
              type="button"
              onClick={handleClose}
              className="w-full cursor-pointer rounded-xl bg-gradient-to-r from-[#D29A45] to-[#E3C49A] px-4 py-2.5 text-sm font-semibold text-[#1a1408] shadow-lg shadow-[#D29A45]/25 transition-all duration-300 hover:from-[#B4522C] hover:to-[#B4522C] active:scale-[0.98]"
            >
              Close
            </button>
          )}
