...
const statusStyles: Record<string, string> = {
  ...
  active: 'bg-[#D29A45]/10 text-[#E3C49A]',
}
    ...
    <div className="min-h-screen bg-background p-0">
      ...
      <div className="overflow-hidden rounded-2xl border border-[#3D2715] bg-gradient-to-b from-[#331E10] to-[#24140B] shadow-xl shadow-black/20">
          ...
          <table className="w-full border-collapse">
            <thead>
              <tr className="border-b border-[#3D2715] bg-white/[0.02]">
                <th className="px-5 py-4 text-left text-xs font-semibold uppercase tracking-wider text-[#C09A76] max-lg:min-w-[240px]">Game Name</th>
                <th className="px-5 py-4 text-left text-xs font-semibold uppercase tracking-wider text-[#C09A76] max-lg:min-w-[180px]">Player</th>
                <th className="px-5 py-4 text-left text-xs font-semibold uppercase tracking-wider text-[#C09A76] max-lg:min-w-[120px]">Seat</th>
                <th className="px-5 py-4 text-left text-xs font-semibold uppercase tracking-wider text-[#C09A76] max-lg:min-w-[140px]">Result</th>
                <th className="px-5 py-4 text-left text-xs font-semibold uppercase tracking-wider text-[#C09A76] max-lg:min-w-[200px]">Prize Won</th>
                <th className="px-5 py-4 text-left text-xs font-semibold uppercase tracking-wider text-[#C09A76] max-lg:min-w-[150px]">Date</th>
                <th className="px-5 py-4 text-right text-xs font-semibold uppercase tracking-wider text-[#C09A76] max-lg:min-w-[130px]">Actions</th>
              </tr>
            </thead>
            <tbody>
              {isLoading ? (
                <tr>
                  <td colSpan={7}>
                    <div className="flex flex-col items-center justify-center gap-3 py-16">
                      ...
                      <p className="text-sm text-[#C09A76]">Loading history...</p>
                    </div>
                  </td>
                </tr>
              ) : isError ? (
                <tr>
                  <td colSpan={7}>
                    <div className="py-16 text-center">
                      <p className="text-sm text-[#C09A76]">Failed to load game history.</p>
                    </div>
                  </td>
                </tr>
              ) : (
                filteredHistory.map((entry) => {
                  ...
                  return (
                    <tr key={entry._id} className="border-b border-[#3D2715]/60 transition-colors duration-300 last:border-0 hover:bg-white/[0.03]">
                      <td className="px-5 py-4">
                        <p className="text-sm font-medium text-[#F4EADD]">{entry.gameTitle}</p>
                        <p className="mt-0.5 font-plus text-xs text-[#9A7A5C]">{entry.gameCode}</p>
                      </td>
                      <td className="px-5 py-4">
                        <p className="text-sm font-medium text-[#F4EADD]">{entry.userName}</p>
                        <p className="mt-0.5 text-xs text-[#9A7A5C]">{entry.userPhone}</p>
                      </td>
                      <td className="px-5 py-4 text-sm text-[#C09A76]">#{entry.seatNumber}</td>
                      <td className="px-5 py-4">
                        <span
                      ...
                      <td className="px-5 py-4 text-right">
                        <button
                          ...
                          disabled={isDeleting}
                          className="inline-flex h-8 w-8 cursor-pointer items-center justify-center rounded-lg border border-[#3D2715] bg-[#24140B] text-[#9A7A5C] transition-colors duration-300 hover:border-orange-600/30 hover:bg-orange-600/10 hover:text-amber-500 disabled:cursor-not-allowed disabled:opacity-40"
        ...
        {!isLoading && !isError && filteredHistory.length === 0 && (
          <div className="py-16 text-center">
            <p className="text-sm text-[#C09A76]">No winners found.</p>
          </div>
        )}
...
function StatItem({ label, value, accent = '' }: StatItemProps) {
  return (
    <div className="rounded-2xl border border-[#3D2715] bg-gradient-to-b from-[#331E10] to-[#24140B] p-5 shadow-xl shadow-black/20">
      <p className="mb-1.5 text-xs font-medium text-[#C09A76]">{label}</p>
      <p className={`text-2xl font-bold ${accent || 'text-[#F4EADD]'}`}>{value}</p>
    </div>
  )
}
