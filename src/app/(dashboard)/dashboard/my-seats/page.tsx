    ...
    <div className="min-h-screen bg-background p-0">
      ...
      <div className="mb-8">
        <div className="mb-4 flex items-center gap-2">
          <Ticket size={18} className="text-[#C78C3A]" />
          <h2 className="text-xl font-bold text-[#F2E8DC]">Reserved Seats</h2>
          {pendingCount > 0 && (
        ...
        {isLoading ? (
          <div className="space-y-3">
            {Array.from({ length: 2 }).map((_, i) => (
              <div
                key={i}
                className="h-[76px] animate-pulse rounded-2xl border border-[#2E1C0E] bg-gradient-to-b from-[#241409] to-[#1B0F08]"
              />
            ))}
          </div>
        ) : pagedSeats.length > 0 ? (
          <div className="space-y-3">
            {pagedSeats.map((game, index) => {
              ...
              return (
                <div
                  key={game.gameId}
                  className="flex items-center justify-between rounded-2xl border border-[#2E1C0E] bg-gradient-to-b from-[#241409] to-[#1B0F08] p-5 shadow-xl shadow-black/20 transition-colors duration-300 hover:border-[#C78C3A]/60"
                >
                  <div className="flex items-center gap-4">
                    <div className="flex h-11 w-11 items-center justify-center rounded-xl bg-[#C78C3A]/10 text-sm font-bold text-[#D0AE95]">
                    ...
                    <div>
                      ...
                      <div className="mt-1 flex items-center gap-2 text-sm text-[#B08A6C]">
                        <Calendar size={14} />
                        {game.joinedAt
                  ...
                  <div className="text-right">
                    <p className="mb-2 text-xs font-medium text-[#B08A6C]">Status</p>
                    <span
          ...
          <div className="rounded-2xl border border-[#2E1C0E] bg-gradient-to-b from-[#241409] to-[#1B0F08] py-12 text-center shadow-xl shadow-black/20">
            <Ticket className="mx-auto mb-3 text-[#B08A6C] opacity-50" size={26} />
      ...
      {/* Recent Game History */}
      <div>
        <h2 className="mb-4 text-xl font-bold text-[#F2E8DC]">Game History</h2>
        <div className="overflow-hidden rounded-2xl border border-[#2E1C0E] bg-gradient-to-b from-[#241409] to-[#1B0F08] shadow-xl shadow-black/20">
            ...
            <table className="w-full border-collapse">
              <thead>
                <tr className="border-b border-[#2E1C0E] bg-white/[0.02]">
                  <th className="px-5 py-4 text-left text-xs font-semibold uppercase tracking-wider text-[#B08A6C] max-lg:min-w-[280px]">Game</th>
                  <th className="px-5 py-4 text-left text-xs font-semibold uppercase tracking-wider text-[#B08A6C] max-lg:min-w-[120px]">Seat</th>
                  <th className="px-5 py-4 text-left text-xs font-semibold uppercase tracking-wider text-[#B08A6C] max-lg:min-w-[150px]">Result</th>
                  <th className="px-5 py-4 text-left text-xs font-semibold uppercase tracking-wider text-[#B08A6C] max-lg:min-w-[180px]">Prize</th>
                  <th className="px-5 py-4 text-left text-xs font-semibold uppercase tracking-wider text-[#B08A6C] max-lg:min-w-[150px]">Joined</th>
                </tr>
              </thead>
              <tbody>
                {isLoading ? (
                  Array.from({ length: 3 }).map((_, i) => (
                    <tr key={i} className="border-b border-[#2E1C0E]/60 last:border-0">
                  ...
                  pagedGames.map((game) => {
                    ...
                    return (
                      ...
                      >
                        <td className="px-5 py-4 text-sm font-medium text-[#F2E8DC]">{game.title}</td>
                        <td className="px-5 py-4">
                          <div className="flex flex-col items-start gap-1.5">
                            <span className="rounded-lg bg-[#C78C3A]/10 px-2.5 py-1 text-xs font-bold text-[#D0AE95]">
                        ...
                        <td className="px-5 py-4 text-sm font-semibold text-[#F2E8DC]">{game.prize}</td>
          ...
          {!isLoading && joinedGames.length === 0 && (
            <div className="py-16 text-center">
              <p className="text-sm text-[#B08A6C]">No game history yet. Join a game to get started.</p>
            </div>
          )}
...
function SummaryCard({ title, value, icon: Icon, tint }: SummaryCardProps) {
  return (
    <div className="flex items-center gap-4 rounded-2xl border border-[#2E1C0E] bg-gradient-to-b from-[#241409] to-[#1B0F08] p-5 shadow-xl shadow-black/20">
      ...
      <div>
        <p className="text-xs font-medium text-[#B08A6C]">{title}</p>
        <p className="mt-0.5 text-2xl font-bold text-[#F2E8DC]">{value}</p>
      </div>
    </div>
  )
}
