    ...
    <div className="min-h-screen bg-background p-0">
      ...
      <div className="mb-8">
        <div className="mb-4 flex items-center justify-between">
          <h2 className="text-xl font-bold text-[#F4EADD]">Featured Games</h2>
          <Link
            href="/dashboard/active-games"
            prefetch={false}
            className="flex items-center gap-1 text-sm font-semibold text-[#D29A45] transition-colors duration-300 hover:text-[#E3C49A]"
          >
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          {gamesLoading ? (
            ...
            <div className="col-span-full rounded-2xl border border-[#3D2715] bg-gradient-to-b from-[#331E10] to-[#24140B] py-14 text-center shadow-xl shadow-black/20">
              <Gamepad2 className="mx-auto mb-3 text-[#C09A76] opacity-50" size={28} />
      ...
      {/* Quick Actions */}
      <div>
        <h2 className="text-xl font-bold text-[#F4EADD] mb-4">Quick Actions</h2>
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          {quickActions.map((action) => (
            <Link
              ...
              href={action.href}
              prefetch={false}
              className={`group rounded-2xl border border-[#3D2715] bg-gradient-to-b from-[#331E10] to-[#24140B] p-6 text-center shadow-xl shadow-black/20 transition-all duration-300 hover:-translate-y-0.5 hover:shadow-lg ${action.ring}`}
            >
              ...
              <h3 className="font-semibold text-[#F4EADD]">{action.title}</h3>
              <p className="mt-1 text-sm text-[#C09A76]">{action.subtitle}</p>
              <span className="mt-3 inline-flex items-center gap-1 text-xs font-semibold text-[#D29A45] opacity-0 transition-opacity group-hover:opacity-100">
  ...
  return (
    <div className="rounded-2xl border border-[#3D2715] bg-gradient-to-b from-[#331E10] to-[#24140B] p-5 shadow-xl shadow-black/20">
      <div className="flex items-start justify-between">
        <div>
          <p className="mb-1.5 text-xs font-medium text-[#C09A76]">{title}</p>
...
function GameCard({ game, isJoined }: GameCardProps) {
  return (
    <div className="group rounded-2xl border border-[#3D2715] bg-gradient-to-b from-[#331E10] to-[#24140B] p-5 shadow-xl shadow-black/20 transition-all duration-300 hover:border-[#D29A45]/60">
      ...
      <div className="mb-5 mt-4 space-y-2.5 border-t border-[#3D2715]/60 pt-4">
      ...
      {isJoined ? (
        ...
        <Link
          href="/dashboard/active-games"
          prefetch={false}
          className="block w-full cursor-pointer rounded-xl bg-gradient-to-r from-[#D29A45] to-[#E3C49A] py-2.5 text-center text-sm font-semibold text-[#1a1408] shadow-lg shadow-[#D29A45]/25 transition-all duration-300 hover:from-[#B4522C] hover:to-[#B4522C] active:scale-[0.98]"
        >
          Join Game
        </Link>
      )}
